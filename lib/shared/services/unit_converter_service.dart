...
class UnitConverterService {
  static const String _openFoodFactsBaseUrl = 'https://world.openfoodfacts.org/cgi/search.pl';
  ...
  static Future<double> convertToGrams(
    ...
    if (amount <= 0 || amount > 1000) {
      _logger.w('Invalid amount $amount $unit for $ingredientName, defaulting to 100g');
      return 100.0;
    }
    ...
    if (usdaPortions != null && usdaPortions.isNotEmpty) {
      final usdaGrams = _convertUsingUSDAPortions(amount, unitLower, usdaPortions, ingredientName);
      if (usdaGrams != null) {
        return usdaGrams;
      }
    }
    ...
    final fallbackGrams = await _getOpenFoodFactsPortion(ingredientName, amount, unit);
    if (fallbackGrams != null) {
      _logger.i('Converted $amount $unit to $fallbackGrams g for $ingredientName using Open Food Facts');
  ...
  static double? _convertUsingUSDAPortions(
    ...
    final portion = portions.firstWhere(
      (p) => p['measureUnit'] != null && p['measureUnit']['name'].toLowerCase() == unit,
      orElse: () => <String, dynamic>{},
    );

    if (portion.isNotEmpty && portion['gramWeight'] != null && portion['gramWeight'] > 0) {
      final grams = amount * portion['gramWeight'].toDouble();
      _logger.i('Converted $amount $unit to $grams g for $ingredientName using USDA portion');
  ...
  static Future<double?> _getOpenFoodFactsPortion(String ingredientName, double amount, String unit) async {
    try {
      final query = _normalizeQuery(ingredientName);
      final response = await http.get(
        Uri.parse('$_openFoodFactsBaseUrl?search_terms=${Uri.encodeComponent(query)}&json=1&page_size=10'),
      );
      ...
      if (response.statusCode == 200) {
        ...
        if (products != null && products.isNotEmpty) {
          ...
          if (servingSize != null && servingSize.contains('g')) {
            final grams = double.tryParse(servingSize.replaceAll(RegExp(r'[^\d.]'), '')) ?? 100.0;
  ...
  static double _convertUsingDensity(double amount, String unit, String ingredientName) {
    ...
    switch (unit) {
      ...
      case 'milliliters':
        ...
        _logger.w('Converted $amount ml to $grams g for $ingredientName using density $density');
        return grams;
      case 'cup':
      case 'cups':
        final grams = amount * density * 240.0; // 240ml per cup
        _logger.w('Using density-based cup conversion ($density * 240g) for $ingredientName: $grams g');
        return grams;
      case 'tbsp':
      case 'tablespoon':
      case 'tablespoons':
        final grams = amount * density * 15.0; // 15ml per tbsp
        _logger.w('Using density-based tbsp conversion ($density * 15g) for $ingredientName: $grams g');
        return grams;
      case 'tsp':
      case 'teaspoon':
      case 'teaspoons':
        final grams = amount * density * 5.0; // 5ml per tsp
        _logger.w('Using density-based tsp conversion ($density * 5g) for $ingredientName: $grams g');
        return grams;
      default:
  ...
  static double _estimateDensity(String ingredientLower) {
    if (ingredientLower.contains('oat') || ingredientLower.contains('quinoa') || ingredientLower.contains('grain')) {
      return 0.5; // Grains are less dense
    } else if (ingredientLower.contains('lentil') || ingredientLower.contains('bean') || ingredientLower.contains('legume')) {
      return 0.8; // Legumes are moderately dense
    } else if (ingredientLower.contains('oil') || ingredientLower.contains('fat')) {
      return 0.9; // Oils are dense
    } else if (ingredientLower.contains('vegetable') || ingredientLower.contains('fruit')) {
  ...
  static double _estimatePortionMultiplier(String unit, Map<String, dynamic> product) {
  ...
  static String _normalizeQuery(String query) {
    ...
    normalized = normalized.replaceAll(RegExp(r'[^\w\s]'), ' ').replaceAll(RegExp(r'\s+'), ' ');
    return normalized;
  }
} 