...
class OpenFoodFactsService {
  ...
  static Future<OpenFoodFactsResult?> searchAndNormalize(String ingredientName) async {
    try {
      print('🔍 Open Food Facts: Searching for "$ingredientName"');
      
      final response = await http.get(
        Uri.parse('$_baseUrl?search_terms=${Uri.encodeComponent(ingredientName)}&json=1&page_size=5'),
  ...
  static OpenFoodFactsResult? _parseSearchResults(Map<String, dynamic> data, String originalQuery) {
    ...
    print('📋 Open Food Facts: Found ${products.length} products for "$originalQuery"');
    ...
    if (bestMatch != null) {
      print('🎯 Open Food Facts: Best match for "$originalQuery" -> "${bestMatch.productName}" (score: $bestScore)');
  ...
  static double _calculateRelevanceScore(Map<String, dynamic> product, String originalQuery) {
    double score = 0;
    ...
    if (productName.contains(originalLower) || originalLower.contains(productName)) {
      score += 50;
    }
    
    // Partial word matches
    final originalWords = originalLower.split(RegExp(r'[,\s]+')).where((w) => w.isNotEmpty);
  ...
  static String _normalizeProductName(String productName) {
    ...
    final qualifiersToRemove = [
      'organic', 'gluten free', 'non-gmo', 'natural', 'fresh', 'raw',
      'whole grain', 'whole wheat', 'extra virgin', 'pure', 'la choy',
      'gerble', 'galettes', 'sudoises', 'chocolat', 'cookie', '132g', '47oz',
      '15 fl oz', '15 oz', 'light', 'super firm', 'cubed', 'sprouted'
    ];
  ...
  static String _extractCoreIngredient(String productName) {
    ...
    final ingredientKeywords = [
      'oat', 'oats', 'tofu', 'spinach', 'soy sauce', 'sesame', 'seed',
      'lentil', 'almond', 'blueberry', 'chicken', 'rice', 'quinoa'
    ];
    ...
    for (final word in words) {
      if (word.isNotEmpty && ingredientKeywords.any((keyword) => word.contains(keyword))) {
        coreWords.add(word);
      }
    }
  ...
  static Map<String, double>? getFallbackNutrition(OpenFoodFactsProduct product) {
