...
class NutritionComparisonService {
  ...
  static NutritionComparison compareNutrition(
    ...
    for (final nutrient in ['calories', 'protein', 'carbs', 'fat']) {
      if (claimed.containsKey(nutrient) && expected.containsKey(nutrient)) {
        ...
        if (expectedValue > 0) {
          final difference =
              (claimedValue - expectedValue).abs() / expectedValue;
    ...
    final toleranceConfidence =
        totalNutrients > 0 ? matches / totalNutrients : 0.0;
    final isWithinTolerance = toleranceConfidence >= 0.7;
    final usdaDataQuality =
        expected.values.where((v) => v > 0).length / expected.length;
    final confidence = usdaDataQuality * 0.8 + toleranceConfidence * 0.2;

    String message;
    if (isWithinTolerance) {
      message =
          'Nutrition data verified with ${(toleranceConfidence * 100).toInt()}% tolerance match';
    } else {
      message =
          'Nutrition data may need adjustment. ${(toleranceConfidence * 100).toInt()}% of nutrients within tolerance.';
    }

    _logger.i(
        'Nutrition comparison: $toleranceConfidence tolerance, $confidence confidence, ${corrections.length} corrections needed');
  ...
  static Map<String, double> calculateNutritionForAmount(
    ...
    return {
      'calories': ((nutritionPer100g['calories'] ?? 0.0) * multiplier)
          .clamp(0.0, 1000.0),
      'protein':
          ((nutritionPer100g['protein'] ?? 0.0) * multiplier).clamp(0.0, 200.0),
      'carbs':
          ((nutritionPer100g['carbs'] ?? 0.0) * multiplier).clamp(0.0, 300.0),
      'fat': ((nutritionPer100g['fat'] ?? 0.0) * multiplier).clamp(0.0, 100.0),
      'fiber':
          ((nutritionPer100g['fiber'] ?? 0.0) * multiplier).clamp(0.0, 50.0),
      'sugar':
          ((nutritionPer100g['sugar'] ?? 0.0) * multiplier).clamp(0.0, 100.0),
      'sodium':
          ((nutritionPer100g['sodium'] ?? 0.0) * multiplier).clamp(0.0, 2000.0),
    };
  }

  /// Validate nutrition data for reasonableness
  static bool validateNutritionData(
      Map<String, double> nutrition, String ingredientName) {
    ...
    if (calorieDiff > calories * 0.3) {
      // 30% tolerance
      _logger.w(
          'Calorie mismatch for $ingredientName: claimed $calories, calculated $calculatedCalories');
      return false;
    }
  ...
  static Map<String, dynamic> getNutritionSummary(
    ...
    return {
      ...
      'totalMacros': (nutrition['protein'] ?? 0.0) +
          (nutrition['carbs'] ?? 0.0) +
          (nutrition['fat'] ?? 0.0),
    };
  }
